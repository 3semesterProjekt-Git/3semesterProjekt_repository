...
/*******************************************************************************
* Function Name: Mixer_1_aclk_Start
********************************************************************************
*
* Summary:
...
void Mixer_1_aclk_Start(void) 
{
    ...
    Mixer_1_aclk_CLKEN |= Mixer_1_aclk_CLKEN_MASK;
	Mixer_1_aclk_CLKSTBY |= Mixer_1_aclk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Mixer_1_aclk_Stop
********************************************************************************
*
* Summary:
...
void Mixer_1_aclk_Stop(void) 
{
    ...
    Mixer_1_aclk_CLKEN &= (uint8)(~Mixer_1_aclk_CLKEN_MASK);
	Mixer_1_aclk_CLKSTBY &= (uint8)(~Mixer_1_aclk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Mixer_1_aclk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Mixer_1_aclk_CLKEN & Mixer_1_aclk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Mixer_1_aclk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Mixer_1_aclk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Mixer_1_aclk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Mixer_1_aclk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Mixer_1_aclk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Mixer_1_aclk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Mixer_1_aclk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Mixer_1_aclk_GetSourceRegister();
    uint16 oldDivider = Mixer_1_aclk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Mixer_1_aclk_CLKEN & Mixer_1_aclk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Mixer_1_aclk_DIV_PTR, clkDivider);
                Mixer_1_aclk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Mixer_1_aclk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Mixer_1_aclk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Mixer_1_aclk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Mixer_1_aclk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Mixer_1_aclk_DIV_PTR);
}


/*******************************************************************************
* Function Name: Mixer_1_aclk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Mixer_1_aclk_MOD_SRC |= modeBitMask & (uint8)Mixer_1_aclk_MODE_MASK;
}


/*******************************************************************************
* Function Name: Mixer_1_aclk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Mixer_1_aclk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Mixer_1_aclk_MODE_MASK));
}


/*******************************************************************************
* Function Name: Mixer_1_aclk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Mixer_1_aclk_MOD_SRC & (uint8)(Mixer_1_aclk_MODE_MASK);
}


/*******************************************************************************
* Function Name: Mixer_1_aclk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Mixer_1_aclk_GetDividerRegister();
    uint8 oldSrc = Mixer_1_aclk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Mixer_1_aclk_MOD_SRC |= CYCLK_SSS;
        Mixer_1_aclk_MOD_SRC =
            (Mixer_1_aclk_MOD_SRC & (uint8)(~Mixer_1_aclk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Mixer_1_aclk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Mixer_1_aclk_MOD_SRC & Mixer_1_aclk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Mixer_1_aclk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Mixer_1_aclk_PHASE = clkPhase & Mixer_1_aclk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Mixer_1_aclk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Mixer_1_aclk_PHASE & Mixer_1_aclk_PHASE_MASK;
}
